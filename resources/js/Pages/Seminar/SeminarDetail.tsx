// resources/js/Pages/Seminars/SeminarDetail.tsx  (adjust path to match yours)
...
export default function SeminarDetail() {
  ...
  const { seminar, lang, pageSeo } = usePage<{
    seminar: Seminar;
    lang: "en" | "ja";
  ...
  const getTitle = () =>
    lang === "en" ? seminar.title : seminar.title_ja || seminar.title;

  const getDescription = () =>
    lang === "en" ? seminar.description : seminar.description_ja || seminar.description;

  const getLocation = () =>
    lang === "en" ? seminar.location : seminar.location_ja || seminar.location;
  ...
  const eventData = [
    ...
    ...(seminar.participation_fee
      ? [{
          ...
          label_jp: "参加費",
          value: [lang === "en" ? seminar.participation_fee : seminar.participation_fee_ja || seminar.participation_fee],
        }]
      : []),
    ...(seminar.organizer
      ? [{
          label_en: "Organizer",
          label_jp: "主催",
          value: [lang === "en" ? seminar.organizer : seminar.organizer_ja || seminar.organizer],
        }]
      : []),
    ...(seminar.sponsorship
      ? [{
          ...
          label_jp: "協賛",
          value: [lang === "en" ? seminar.sponsorship : seminar.sponsorship_ja || seminar.sponsorship],
        }]
      : []),
    ...(seminar.cooperation
      ? [{
          ...
          label_jp: "協力",
          value: [lang === "en" ? seminar.cooperation : seminar.cooperation_ja || seminar.cooperation],
        }]
      : []),
  ];
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="max-w-7xl mx-auto bg-white shadow-md border border-border rounded-xl overflow-hidden">
            {eventData.map((item, index) => (
              <div
                key={index}
                className={`grid grid-cols-3 ${index !== eventData.length - 1 ? "border-b border-border" : ""}`}
              >
                <div className="bg-section-light p-6 font-semibold text-gray-700 text-center">
                  {lang === "en" ? item.label_en : item.label_jp}
                </div>
