...
//   2. Added SeoFields component inside the Add/Edit sheet (after the FAQs section)
//   3. Added og_image handling in submitAdd / submitUpdate FormData
...
export default function AdminBlogIndex() {
  ...
  const toPageFaq = (r: any): PageFaq => ({
    question: r.question ?? "", question_ja: r.question_ja ?? "",
    answer: r.answer ?? "", answer_ja: r.answer_ja ?? "",
  });

  const openAdd = () => { reset(); setMode("add"); setCurrent(null); setOpen(true); };

  const openEdit = (blog: Blog) => {
    setMode("edit"); setCurrent(blog); setOpen(true);
    setData({
      title: blog.title || "", title_ja: blog.title_ja || "",
      slug: blog.slug || "",
      category: blog.category || "", category_ja: blog.category_ja || "",
      short_description: blog.short_description || "",
      short_description_ja: blog.short_description_ja || "",
      content: blog.content || "", content_ja: blog.content_ja || "",
      author: blog.author || "", author_ja: blog.author_ja || "",
      published_date: blog.published_date || "",
      status: blog.status || "",
      image: null,
      page_faqs: Array.isArray(blog.page_faqs) ? blog.page_faqs.map(toPageFaq) : [],
      // SEO
      meta_title: blog.meta_title || "", meta_title_ja: blog.meta_title_ja || "",
      meta_description: blog.meta_description || "",
      meta_description_ja: blog.meta_description_ja || "",
      meta_keywords: blog.meta_keywords || "",
      meta_keywords_ja: blog.meta_keywords_ja || "",
      og_image: null,
    });
  };
  ...
  const buildFormData = () => {
    ...
    Object.keys(data).forEach((key) => {
      if (key !== "image" && key !== "og_image" && key !== "page_faqs") {
  ...
  const submitAdd = () => {
    router.post(route("admin.blogs.store"), buildFormData(), {
  ...
  const deleteBlog = (id: number) => {
    if (confirm("Delete this blog?")) router.delete(route("admin.blogs.destroy", id));
  };
    ...
    <AuthenticatedLayout header={<h2 className="text-xl font-bold">Blogs</h2>}>
      ...
      <div className="mb-5">
        ...
        <div className="mb-4 max-w-sm relative">
          ...
          <input
            type="text" placeholder="Search Blogs..." value={search}
          ...
          {mode === "view" && current && (
            <div className="space-y-4 mt-6">
              ...
              {current.image && (
                <div>
                  <strong>Image:</strong>
                  <img src={`/storage/${current.image}`} alt="Blog" className="mt-2 h-40 rounded-md border object-contain" />
                </div>
              )}
            ...
            <div className="space-y-5 mt-6">
              ...
              {mode === "edit" && current?.image && (
                <div className="space-y-2">
                  ...
                  <img src={`/storage/${current.image}`} alt="Existing" className="h-32 rounded-md border object-contain" />
                </div>
              )}

              <div className="space-y-1">
                <label className="font-medium">{mode === "edit" ? "Replace Image" : "Upload Image"}</label>
          ...
          {filteredBlogs.map((blog, i) => (
            <TableRow key={blog.id}>
              ...
              <TableCell className="space-x-2 text-center">
                <Button title="View" size="icon" onClick={() => openView(blog)}><Eye className="w-4 h-4" /></Button>
                <Button title="Edit" size="icon" onClick={() => openEdit(blog)}><Pencil className="w-4 h-4" /></Button>
                <Button title="Delete" size="icon" variant="destructive" onClick={() => deleteBlog(blog.id)}><Trash2 className="w-4 h-4" /></Button>
              </TableCell>
            </TableRow>
          ))}
        </TableBody>
