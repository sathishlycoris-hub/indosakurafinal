...
import { Plus, Eye, Pencil, Trash2, ChevronDown, ChevronUp } from "lucide-react";
...
/* per-service — stored in service_page_faqs / service_page_industries */
...
interface Service {
  ...
  /* loaded via pageFaqs / pageIndustries relations */
  pageFaqs?: PageFaq[];
  pageIndustries?: PageIndustry[];
}
...
export default function Index({ services }: { services: Service[] }) {
  ...
  const { data, setData, reset, processing } = useForm({
    ...
    page_faqs: [] as PageFaq[],        // → service_page_faqs
    page_industries: [] as PageIndustry[],   // → service_page_industries
  });

  /* ── open helpers ── */
  const openAdd = () => {
    reset(); setMode("add"); setCurrent(null); setLangTab("en"); setOpen(true);
  };

  const openEdit = (s: Service) => {
    setMode("edit"); setCurrent(s); setLangTab("en"); setOpen(true);
    setData({
      ...
      why_choose: s.why_choose ?? [],
      approach_steps: s.approach_steps ?? [],
      testimonials: s.testimonials ?? [],
      tech_stack: s.tech_stack ?? [],
      page_faqs: s.pageFaqs ?? [],        // relation → form key
      page_industries: s.pageIndustries ?? [],  // relation → form key
    });
  };
  ...
  const submit = () => {
    ...
    append("title", data.title); append("title_ja", data.title_ja);
    append("slug", data.slug);
    append("subtitle", data.subtitle); append("subtitle_ja", data.subtitle_ja);
    ...
    append("overview", data.overview); append("overview_ja", data.overview_ja);
    append("cta_label", data.cta_label); append("cta_label_ja", data.cta_label_ja);
    ...
    const arrays: (keyof typeof data)[] = [
      ...
      "page_faqs",       // → controller decodes as page_faqs
      "page_industries", // → controller decodes as page_industries
    ];
    arrays.forEach(k => form.append(k as string, JSON.stringify(data[k])));

    const opts = { onSuccess: () => { reset(); setOpen(false); } };
        ...
        <SheetContent className="w-[95%] sm:max-w-5xl overflow-y-auto">
          ...
          {mode === "view" && current && (
            <div className="space-y-6 mt-6 text-sm">
              ...
              {/* <ViewField label="Hero Description" en={current.hero_description} ja={current.hero_description_ja} html /> */}
              <ViewField label="Overview" en={current.overview} ja={current.overview_ja} html />
              <ViewField label="How It Works" en={current.how_it_works} ja={current.how_it_works_ja} html />
              {current.cta_label && (
                <div>
              ...
              {/* per-service relations */}
              <ViewList label="Page FAQs (service_page_faqs)" items={current.pageFaqs} isFaq />
              <ViewList label="Page Industries (service_page_industries)" items={current.pageIndustries} />
            </div>
          )}
            ...
            <div className="space-y-5 mt-6">
              ...
              <Tabs value={langTab} onValueChange={(v) => setLangTab(v as "en" | "ja")}>
                ...
                <TabsContent value="en" className="space-y-4">
                  <Field label="Title (EN)">
                    <Input value={data.title} onChange={e => setData("title", e.target.value)} placeholder="e.g. AI-Driven Development" />
                  </Field>
                  <Field label="Slug">
                    <Input value={data.slug} onChange={e => setData("slug", e.target.value)} placeholder="ai-driven-development" />
                  </Field>
                  <Field label="Subtitle (EN)">
                    <Input value={data.subtitle} onChange={e => setData("subtitle", e.target.value)} placeholder="Build Intelligent, Scalable & Future-Ready AI Solutions" />
                  </Field>
                  {/* <Field label="Hero Description (EN)">
                    <ReactQuill value={data.hero_description} onChange={v => setData("hero_description", v)} />
                  </Field> */}
                  <Field label="Overview (EN)">
                    <ReactQuill value={data.overview} onChange={v => setData("overview", v)} />
                  </Field>
                  <Field label="How It Works (EN)">
                    <ReactQuill value={data.how_it_works} onChange={v => setData("how_it_works", v)} />
                  </Field>
                </TabsContent>
                ...
                <TabsContent value="ja" className="space-y-4">
                  ...
                  {/* <Field label="Hero Description (JA)">
                    <ReactQuill value={data.hero_description_ja} onChange={v => setData("hero_description_ja", v)} />
                  </Field> */}
                  <Field label="Overview (JA)">
                    <ReactQuill value={data.overview_ja} onChange={v => setData("overview_ja", v)} />
                  </Field>
                  <Field label="How It Works (JA)">
                    <ReactQuill value={data.how_it_works_ja} onChange={v => setData("how_it_works_ja", v)} />
                  </Field>
                </TabsContent>
              </Tabs>
                ...
                render={(item, i) => (
                  <div className="space-y-2">
                    <div className="grid grid-cols-2 gap-2">
                      <Field label="Stat value"><Input value={item.value} placeholder="51.3%" onChange={e => updateItem("highlights", i, "value", e.target.value)} /></Field>
                      <Field label="Title (EN)"><Input value={item.title} placeholder="Reduction in development time" onChange={e => updateItem("highlights", i, "title", e.target.value)} /></Field>
                    </div>
                ...
                render={(item, i) => (
                  <div className="space-y-2">
                    <div className="grid grid-cols-3 gap-2">
                      ...
                      <div className="col-span-2"><Field label="Title (EN)"><Input value={item.title} placeholder="AI Strategy & Consulting" onChange={e => updateItem("approach_steps", i, "title", e.target.value)} /></Field></div>
                    </div>
              ...
              <SectionBlock title="Technology Stack" hint="e.g. AI/ML Frameworks: TensorFlow, PyTorch"
                ...
                render={(item, i) => (
                  <div className="space-y-2">
                    <div className="grid grid-cols-2 gap-2">
                      <Field label="Category (EN)"><Input value={item.category} placeholder="AI/ML Frameworks" onChange={e => updateItem("tech_stack", i, "category", e.target.value)} /></Field>
                      <Field label="Category (JA)"><Input value={item.category_ja || ""} onChange={e => updateItem("tech_stack", i, "category_ja", e.target.value)} /></Field>
                    </div>
                    <Field label="Items (comma-separated)"><Input value={item.items} placeholder="TensorFlow, PyTorch, Scikit-learn" onChange={e => updateItem("tech_stack", i, "items", e.target.value)} /></Field>
                  </div>
                )}
              />
              ...
              <SectionBlock title="Testimonials" hint="Client quotes"
                ...
                render={(item, i) => (
                  <div className="space-y-2">
                    <Field label="Author / Company"><Input value={item.author || ""} placeholder="Jane Doe, Acme Corp" onChange={e => updateItem("testimonials", i, "author", e.target.value)} /></Field>
              ...
              {/* ══════════════════════════════════════════════════════
                  ...
                  Leave empty → public page falls back to global panel
              ══════════════════════════════════════════════════════ */}
              <SectionBlock
                title="Industries We Serve (per-service)"
                hint="Stored in service_page_industries — leave empty to use the global Industries panel"
              ...
              <SectionBlock
                ...
                hint="Stored in service_page_faqs — leave empty to use the global FAQ panel"
    ...
    <div className="border rounded-xl bg-muted/20 overflow-hidden">
      <button type="button" onClick={() => setCollapsed(c => !c)}
        ...
        <div>
          ...
          <span className="ml-2 text-xs bg-primary/10 text-primary px-2 py-0.5 rounded-full">{items.length}</span>
        </div>
...
function ViewList({ label, items, showValue = false, showStep = false,
