...
interface SiteSettings {
  ...
  footer_india_desks_heading_en: string; footer_india_desks_heading_ja: string; // ★ NEW
...
const DEFAULTS: SiteSettings = {
  ...
  footer_india_desks_heading_en: "India Desks", footer_india_desks_heading_ja: "インドデスク", // ★ NEW
      ...
      <div className="container py-8 md:py-12">
        <div className="flex flex-col lg:flex-row gap-6 md:gap-8 lg:gap-1 mb-8 md:mb-12">
          <div className="flex-1">
            {/* ★ Changed to grid-cols-3 md:grid-cols-4 lg:grid-cols-5 to fit the new column */}
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6 sm:gap-8 lg:gap-3">
              ...
              {indiaDesks.length > 0 && (
                <div>
                  <h2 className={headingClass}>{t(s.footer_india_desks_heading_en, s.footer_india_desks_heading_ja)}</h2>
                  <ul className="space-y-1.5 sm:space-y-2">
                    {indiaDesks.map((desk) => (
                      <li key={desk.slug}>
                        <Link href={`/india-desks/${desk.slug}`} className={linkClass}>{tv(desk.title, desk.title_ja)}</Link>
                      </li>
                    ))}
                  </ul>
                </div>
              ...
              <div>
                ...
                <ul className="space-y-1.5 sm:space-y-2">
                  {solutions.map((sol) => (
                    <li key={sol.slug}>
                      <Link href={`/solutions/${sol.slug}`} className={linkClass}>{tv(sol.title, sol.title_ja)}</Link>
                    </li>
                  ))}
                </ul>
              </div>
              ...
              <div>
                ...
                <ul className="space-y-1.5 sm:space-y-2">
                  {services.map((svc) => (
                    <li key={svc.slug}>
                      <Link href={`/services/${svc.slug}`} className={linkClass}>{tv(svc.title, svc.title_ja)}</Link>
                    </li>
                  ))}
                </ul>
              </div>
          ...
          {/* Footer logo */}
          <div>
            <img src={footerLogoSrc} alt="20 years of excellence" className="w-16 sm:w-20 object-contain" />
          </div>
        </div>
